...
const submitActionZip = useCallback(() => {
  try{
    const data = zip.filter(({label}:any) => label.toLowerCase().includes(codes.toLowerCase()));
    console.log(codes, zipObj);
    setCodes(data[0].label);
    setZipBool(true);
    return;
  ...
  },[submitAction, search, codes, zipObj, submitActionZip]);
  return (
