...
const AutoSuggestInput = ({autocomplete = [], placeholder='', input='', setInput}:any) => {
  ...
  const arrGroup = group.map((e:any) => JSON.stringify(e) );
  const result = currentSort.map((e:any) => JSON.stringify(e)).filter((e:any) => !arrGroup.includes(e) ).map((e:any)=> JSON.parse(e));
  return sorting;
  }
