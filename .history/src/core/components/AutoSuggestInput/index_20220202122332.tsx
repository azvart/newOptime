...
const AutoSuggestInput = ({autocomplete = [], placeholder='', input='', setInput}:any) => {
  ...
  const sorting = data.map(({label,title}:any) => {
    return {title,label};
...
//  const grouping = sorting.map((items:any) => autocomplete.find((item:any) =>  ));
//   console.log(grouping);
  // const group = sorting.map((items:any) => items.label).flat();
  // const grouping = group.map(({label}:any) => {})
  // console.log(group);
  console.log(sorting);
  return sorting;
  }
