...
const AutoSuggestInput = ({autocomplete = []}) => {
  ...
  const getSuggestion = (value:any):any => {
    ...
    //   const datas =  {
    //     title: section.title,
    //     label: [section.label.find((labels:any) => labels.label.toLowerCase().includes(value.toLowerCase()) )]
    //   }
    //   console.log(datas);
    //   return datas;
    // })
    ...
    const data = autocomplete.map((section:any) => {
      return {
        title: section.title,
        label:section.label.filter((item:any) => item.label.toLowerCase().includes(value.toLowerCase()))
      }
    })
