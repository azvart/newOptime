...
const AutocompleteInput: FC<Props> = ({
  ...
  return (
    ...
    <Autocomplete
      ...
      renderItem={(item,isHighlighted)=>grouping === true ? (<div className="none"></div>) : <div className={`autocomplete ${isHighlighted ? 'ishigh' : null}`} key={item.label} ><div className="autocomplete__item">{item.label}</div></div>}
      value={value}
