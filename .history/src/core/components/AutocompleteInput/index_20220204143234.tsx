...
const AutocompleteInput: FC<Props> = ({
  ...
  return (
    ...
    <Autocomplete
      ...
      renderItem={(item,isHighlighted)=>grouping === true ? (<div className="none"></div>) : <div className={`autocomplete`} key={item.label} ><div className={`autocomplete__item`} style={{background : isHighlighted ? '#ff4b9c' : 'white'}}>{item.label}</div></div>}
      value={value}
