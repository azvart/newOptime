...
const AutocompleteInput: FC<Props> = ({
  ...
  useEffect(() => {
    console.log('auto component', value);
    setValue(value);
  },[value]);
  return (
    <div ref={node}>
    <Autocomplete
      ...
      renderItem={(item,isHighlighted)=>grouping === true ? (<div className="none"></div>) : <div className={`autocomplete`} key={item.label} ><div className={`autocomplete__item`} style={{background : isHighlighted ? '#ff4b9c' : 'white', color: isHighlighted ? '#fff': '#7d7d7d' }}>{item.label}</div></div>}
      value={value}
