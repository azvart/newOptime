...
const InputField: FC<Props> = ({
  ...
  useEffect(() => {
    setList(autocomplete);
  },[autocomplete]);
  const onSubmit = useCallback(() => {
    try{
    const data = list.filter(({label}:any) => label === value);
    const listData = autocomplete.filter(({label}:any) => label === value);
    const firstElem = listData.length === 0 ? list[0].label : data[0].label;
    if(data.length === 0){
      onChange(firstElem);
    }
    setBool(true);
